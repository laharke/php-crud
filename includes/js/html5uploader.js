...
function uploader(place, status, targetPHP, show) {
		...
		if(window.FileReader) { 
			...
			this.loadEnd = function() {
				...
				if (show) {
					...
					document.getElementById(show).innerHTML = '<br><b>Archivo</b>: '+file.name+'<br><br><b>Tama&ntilde;o:</b> '+file.size+' b';				
				}
				if (status) {
					document.getElementById(status).innerHTML = '<br><b>Progreso</b>: 100%';
				}
			}
			...
			this.loadProgress = function(event) {
				if (event.lengthComputable) {
					...
					document.getElementById(status).innerHTML = '<br><b>Progreso</b>: '+percentage+'%';
				}				
			}
	...
	this.uploadPlace.addEventListener("dragover", function(event) {
		...
		document.getElementById(place).style.backgroundColor = 'rgba(200, 0, 0, 0.25)';
	}, true);
